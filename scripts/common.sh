...
# ── Extension versions ───────────────────────────────────────────────────────
NVGPU_VERSION="${NVGPU_VERSION:-5.6.0}"          # .../ 5.5.0 (strip -pg/-mrecord-mcount) / 5.6.0 (strip -fpatchable-function-entry=*: fix ftrace crash on ARM64 with MCOUNT_USE_PATCHABLE_FUNCTION_ENTRY)
