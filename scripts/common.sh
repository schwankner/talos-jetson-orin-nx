...
# ── Extension versions ───────────────────────────────────────────────────────
NVGPU_VERSION="${NVGPU_VERSION:-5.10.10}"        # .../ 5.10.7 (fix: SYNCPT_WAITMEX min-timeout 30s, GA10B large-model kernel timeout) / 5.10.8 (chore: switch linux-nv-oot to stable patches-r36.5) / 5.10.9 (chore: remove dead code from fixup.sh — kref_init already upstream, GCC plugin block unreachable with LLVM=1) / 5.10.10 (refactor: replace #if 1 force-hacks with LINUX_VERSION_CODE guards; remove GCC_PLUGIN_LATENT_ENTROPY dead code from nvgpu build)
